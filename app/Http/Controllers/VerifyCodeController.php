...
class VerifyCodeController {
	...
	public function make()
	{
		if(empty($this->font))
		{
			$this->font = __DIR__.'\style\font\fonts\consola.ttf';
		}
		$this->create();//生成验证码
	...
	private function createFont() {
		...
		for ($i = 0; $i < $this->codeLen; $i++) {
			...
			imagettftext($this->img, $this->fontSize, mt_rand(- 30, 30), $x * $i + mt_rand(6, 10), mt_rand($this->height / 1.3, $this->height - 5), $this->fontColor, realpath($this->font), $this->code [$i]);
//            imagettftext($this->img,$this->fontSize,mt_rand(- 30, 30), $x * $i + mt_rand ( 6, 10 ), mt_rand($this->height / 1.3, $this->height - 5), $this->fontColor, realpath($this->font), $this->code [$i]);
